...
def get_stores_to_check(conn, limit=50):
    """获取需要检查的店铺列表"""
    cur = conn.cursor()
...
def main():
    ...
    # 获取要检查的店铺
    print("\n📊 Fetching stores...")
    stores = get_stores_to_check(conn, limit=50)
    ...
    for i, store in enumerate(stores[:5], 1):
        ...
        print(f"  {i}. {domain} ({country}) - {visits_str} visits/month")
    ...
    for i, store in enumerate(stores, 1):
        ...
        # 简洁输出
        print(f"[{i}/{len(stores)}] {domain} ({country}, {visits_str})...", end=" ", flush=True)
        ...
        if has_customily:
            ...
            print(f"    └─ {details}")
            customily_found += 1
            customily_stores.append((domain, country, visits))
        else:
            print(f"⭕")
        ...
        if i % 10 == 0:
            ...
            print(f"\n{'─'*80}")
            print(f"📊 Progress: {i}/{len(stores)} ({i/len(stores)*100:.0f}%) | " +
                  f"Customily: {customily_found} ({customily_found/i*100:.1f}%) | " +
    ...
    if customily_stores:
        print("\n" + "=" * 80)
        print("🎯 安装了 Customily 的店铺:")
        print("=" * 80)
        for domain, country, visits in customily_stores:
            visits_str = f"{visits:,}" if visits else "N/A"
            print(f"  • {domain} ({country}) - {visits_str} visits/month")
    ...
    print("""
1. 查看结果:
   在 Vercel 网站: /api/stores?hasCustomily=true

2. 如果结果正常，可以:
   • 继续测试 100 个: python3 check_customily_test100.py
   • 运行全量检测: python3 check_customily_app.py
   • 部署到 Mac mini 后台运行
    """)
    print("=" * 80)
